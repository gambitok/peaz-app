...
class PostController extends ResponseController
{
    ...
    public function createPost(Request $request)
    {
        ...
        $rules = [
            'title' => ['required'],
            'file' => ['required_if:method,add', 'file'],
            'caption' => ['required'],
        ...
        $tag = Tag::whereIn('name', $request->tags)->exists();
        if ($tag == false) {
        ...
        $request_data['tags'] = implode(",", $request->tags);
    ...
    public function addIngredient(Request $request)
    {
        ...
        $data = null;
        $messages = __('api.suc_ingredient_create');
    ...
    public function addInstruction(Request $request)
    {
        ...
        $this->directValidation($rules);
        $data = null;
        $messages = __('api.suc_instruction_create');
    ...
    public function home(Request $request)
    {
        ...
        if (isset($user) && !empty($user)) {
            ...
            if($c_instance->where('user_id',$user->id)->exists() == true){
                $p_instance->where('not_interested', 0);
            }
        }
